...
import { Activity, Database, Server, Wifi, Container, Hammer } from 'lucide-react';
    ...
    <div className="space-y-4 sm:space-y-6 px-3 sm:px-4 md:px-6 py-4 sm:py-6">
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          <h2 className="text-lg sm:text-xl font-semibold heading-primary">Dashboard</h2>
          <p className="text-xs sm:text-sm heading-secondary mt-0.5">Agent overview and health</p>
        </div>
        <div className={`flex items-center gap-2 text-xs px-3 py-1.5 rounded-full border shrink-0 ${
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="card">
          <h3 className="text-sm font-medium text-slate-300 mb-4 flex items-center gap-2">
            <Server className="w-4 h-4 text-emerald-500/60" />
            Services
          </h3>
          <div className="space-y-2.5">
            <ServiceRow name="HTTP API" Icon={Wifi} ok={!!health} detail={health ? ':13000' : 'offline'} />
            <ServiceRow name="Redis" Icon={Database} ok={health?.services?.redis === 'healthy'} detail={health?.services?.redis ?? 'unknown'} />
            <ServiceRow name="Database" Icon={Database} ok={health?.services?.database === 'healthy'} detail={health?.services?.database ?? 'unknown'} />
            <ServiceRow name="Jenkins" Icon={Hammer} ok={!!health} detail="via REST API" />
            <ServiceRow name="Kubernetes" Icon={Container} ok={!!health} detail="via kubectl" />
          </div>
        </div>
